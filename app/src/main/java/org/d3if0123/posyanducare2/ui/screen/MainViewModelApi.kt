...
class MainViewModelApi : ViewModel() {
    var data = mutableStateOf(emptyList<Kegiatan>())
    private set
    ...
    fun retrieveData(userId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                data.value = Api.service.getHewan(userId)
    ...
    fun saveData(userId: String, nama: String, deskripsi: String, bitmap: Bitmap) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val result = Api.service.postHewan(
                    userId,
    ...
    fun deleteData(userId: String, hewanId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val result = Api.service.deleteHewan(userId, hewanId)
